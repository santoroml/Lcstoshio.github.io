...
# **Contagem de Casos e Morte de Covid-19 no mundo**
...
# Contagem por país/continente em ordem descendente
data %>%
  group_by(country, indicator)%>%
  summarise(soma = sum(cumulative_count)) %>% 
  arrange(desc(soma))

# Total casos/mortes por país/continente em ordem descendente
data %>%
  group_by(country, indicator) %>% 
  summarise(soma = sum(cumulative_count)) %>% 
  arrange(desc(soma))
...
(data %>%
    ...
    labs(title = "Cases and Deaths in the World", # Título, subtítulo e títulos dos eixos
         x = "Date (weeks/year)", 
         y = "Count") + 
...
(data %>% filter(country == "Brazil") %>%
    ...
    theme(legend.position = "none", # Formatação dos textos e eixos
          ...
          axis.text = element_text(family = "Lato", color="black", size = 10, face="bold"),
          axis.text.y = element_text(angle=45))+
    labs(title = "Cases and Deaths in Brazil", # Título, subtítulo e títulos dos eixos
         x = "Date (weeks/year)", 
         y = "Count") + 
     scale_fill_brewer(palette="Set1") + # Cor das barras
     scale_color_brewer(palette="Set1") + # Cor do contorno das barras
...
## **Gráfico de contagens por continente**
...
(data %>%
    ...
    theme(strip.text = element_text(size=12, color="white", family="Bangers"), # Formatação dos textos e eixos
          ...
          axis.text = element_text(family = "Lato", color="black", size = 10, face="bold"),
          axis.text.y = element_text(angle=45))+
    labs(title = "Cases and Deaths by continent (total - stacked)", # Título, subtítulo e títulos dos eixos
         x = "Date (weeks/year)", 
         y = "Count",
         fill = "Continent") + 
     scale_fill_brewer(palette="Set1") + # Cor das barras
...
# Ordenar a coluna continente em fatores em ordem decrescente
#lev <- c(order$continent)
#data$continent <- factor (data$continent, levels = lev)
...
(data %>%
    ...
    theme(strip.text = element_text(size=12, color="white", family="Bangers"), # Formatação dos textos e eixos
          ...
          axis.text = element_text(family = "Lato", color="black", size = 10, face="bold"),
          axis.text.y = element_text(angle=45))+
    labs(title = "Cases and Deaths by continent", # Título, subtítulo e títulos dos eixos
         x = "Date (weeks/year)", 
         y = "Count",
         fill = "Continent") + 
     scale_fill_brewer(palette="Set1") + # Cor das barras
